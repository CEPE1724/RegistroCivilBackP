...
export class NominaService {
  ...
  findOneId(id: number) {
     const cacheKey = `nomina-id:${id}`;
      // Verificar cache
      const cached = this.cacheManager.get<Nomina>(cacheKey);
      if (cached) {
      ...
      const nomina = this.nominaRepository.findOne({ where: { idPersonal: id } });
      // Guardar en cache (30 minutos)
      this.cacheManager.set(cacheKey, nomina, CacheTTL.nominaById);
